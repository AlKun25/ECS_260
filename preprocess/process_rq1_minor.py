...
def main(org):
    ...
    df_devs_nodup = pd.read_csv('devs/combined_developer_nodup_activity_minor_idx.csv')

    df_created = pd.read_csv('repos/selected_repos_created_week.csv')

    # org = input("Org name: ")

    df_commits = pd.read_csv(f'orgs/{org}/{org}_commits_0_bots_removed_nodup_minor_idx.csv')
    df_weekly = pd.read_csv(f'orgs/{org}/{org}_weekly_nodup_minor_idx.csv')
    df_weekly_devs = pd.read_csv(f'orgs/{org}/weekly_dev_activity_bots_removed_nodup_minor_idx.csv')
    ...
    headers = ['org', 'repo', 'weeks_between_minor_minor', 'total_devs', 'multitask_devs', 'singletask_devs', 'multitask_ratio']


    data = []

    for idx in range(1, 1340):
        lines = 0
        modified_files = 0
        idx = float(idx)
        ...
        weeks_between_minor = filtered_df_release['weeks_between_minor_releases']
        if not weeks_between_minor.empty:
            weeks_between_minor = weeks_between_minor.item()
        else:
            weeks_between_minor = None
...
# #             if len(filtered_df_weekly) > 0:
        ...
        data.append([org, unique_repo, weeks_between_minor, total_devs, shared_devs, non_shared_devs, multitask_ratio])


    with open(f'output/_rq1/minor/{org}_processed.csv', mode='w', newline='', encoding='utf-8') as file:
